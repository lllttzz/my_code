...
class Data:
    def __init__(self, name, batch_size):  # 数据所在的文件名和batch中图片的数量batch_size
...
class FullyConnect:
    ...
    def forward(self, x):
        self.x = x  # 把中间结果保存下来，以备反向传播时使用(DATA层传进来的一个batch_size的数据)
        self.y = np.array([np.dot(self.weights, xx) + self.bias for xx in x])  # 计算全连接层的输出(一个batch_size中每一列数据做forward pass)
        return self.y  # 将这一层计算的结果向前传递

    def backward(self, d):
        # ddw保存的是对于每组输入数据，损失函数对于参数的梯度
        ddw = [np.dot(dd, xx.T) for dd, xx in zip(d, self.x)]  # 根据链式法则，将反向传递回来的导数值乘以x，得到对参数的梯度
        self.dw = np.sum(ddw, axis=0) / self.x.shape[0] # w的平均梯度
        self.db = np.sum(d, axis=0) / self.x.shape[0]   # b的平均梯度
...
class Sigmoid:
    ...
    def backward(self, d):
        ...
        self.dx = d * sig * (1 - sig) #sigmoid函数的导数
...
class QuadraticLoss: #损失函数
    ...
    def forward(self, x, label):
        ...
        # (这个self.label不是我们们的标签，而是我们的label，他是预测的输出)
    ...
    def backward(self):
        self.dx = (self.x - self.label) / self.x.shape[0]  # 2被抵消掉了 （导数）
        return self.dx
